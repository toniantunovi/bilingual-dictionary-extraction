...
def train_translation_matrix(dict_file, source_file, target_file, out_file):
    print "Reading the training data"
    train_data = read_dict(dict_file)
    print "Read " + str(len(train_data)) + " training word pairs"
    ...
    source_sp.normalize()
    print "Source space word count: " + str(len(source_sp.id2row))
    

    print "Reading: %s" % target_file
    target_sp = Space.build(target_file, set(target_words))
    target_sp.normalize()
    print "Target space word count: " + str(len(target_words))
    print "Target space word count: " + str(len(target_sp.id2row))
...
def main(sys_argv):
    ...
    train_translation_matrix(dict_file, source_file, target_file, out_file)
